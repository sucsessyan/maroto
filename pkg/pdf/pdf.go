...
type Maroto interface {
	...
	BorderCol(width uint, border string, closure func())
...
func NewMarotoCustomSize(orientation consts.Orientation, pageSize consts.PageSize, unitStr string, width, height float64, watermark bool) Maroto {
...
func NewMaroto(orientation consts.Orientation, pageSize consts.PageSize, watermark bool) Maroto {
	return NewMarotoCustomSize(orientation, pageSize, "mm", 0, 0, watermark)
}
...
// Col create a column with custom border inside a row and enable to add
// components inside.
func (s *PdfMaroto) BorderCol(width uint, border string, closure func()) {
	if width == 0 {
		width = 12
	}
	...
	s.colWidth = widthPerCol
	s.createBorderColSpace(widthPerCol, border)
...
func (s *PdfMaroto) createBorderColSpace(actualWidthPerCol float64, border string) {
